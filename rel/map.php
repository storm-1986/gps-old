...
/*
	$flog = fopen($path."log.txt", "a+b");
//	$delflog = ftruncate ($flog, 0);
	$flogzap = fwrite($flog,date("d.m.Y H:i:s")." - Запрос фактических стоянок\n".$msg);
	fclose($flog);
*/
	include "sock.php";
if (isset ($out)){
/*
	$flog = fopen($path."log.txt", "a+b");
	$flogzap = fwrite($flog, date("d.m.Y H:i:s")." - Данные от Виктора\n".$out);
	fclose($flog);
*/
	$find1 = "<Error>";
	$find2 = "</Error>";
...
$vpresultid = sqlsrv_query($conn, $vpquery);
$vpi = 0;
$sub_list = '';
$pstop_m = "plan_stops_LayerGroup = L.layerGroup();";
//while (ads_fetch_row($vpresultid)){

while($data_vprid = sqlsrv_fetch_array($vpresultid, SQLSRV_FETCH_ASSOC)){
	$vpi++;
	...
	$adr = trim(iconv("CP1251", "UTF-8", $adr));
